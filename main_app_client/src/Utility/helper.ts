 ...
 */
export const replaceAll = (srcString: string, search: string, replacement: string): string => {
...
export const onKeyDown = (e: any) => {
  if (!isNaN(+e.target.value)){
    ...
    if (decimal_index > -1){
      const decimals = e.target.value.substring(decimal_index, e.target.value.length+1);
      if (!!decimals.length && e.keyCode !== 8){
        e.preventDefault();
      } else {
        return true;
      }
...
export const replaceToNumeric = (val: string) => {
